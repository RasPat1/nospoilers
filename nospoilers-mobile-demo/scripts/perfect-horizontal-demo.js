...
class PerfectHorizontalDemo {
  ...
  async createUsers() {
    for (let i = 0; i < 4; i++) {
      ...
      // Add user indicator at bottom with emoji
  ...
  async addMovie(userIndex, movieName) {
    ...
    if (movieButtons[0]) {
      ...
      // Special effect for "Everything Everywhere All at Once"
      if (movieName.includes('Everything Everywhere')) {
        for (const p of this.pages) {
          await p.evaluate(() => {
            const highlight = document.createElement('div');
            highlight.textContent = '✨ Synced everywhere all at once! ✨';
  ...
  async generateVideo() {
    ...
    try {
      ...
      const outputPath = path.join(__dirname, '..', 'output', 'mobile-4user-demo.mp4');
      ...
      const output60s = path.join(__dirname, '..', 'output', 'mobile-4user-demo-60s.mp4');
      const shortCmd = `ffmpeg -y -i "${outputPath}" -t 60 -c copy "${output60s}"`;
      await execPromise(shortCmd);
      console.log('✅ Created 60-second version');
      
      // Create a 30-second version
      const output30s = path.join(__dirname, '..', 'output', 'mobile-4user-demo-30s.mp4');
      const short30Cmd = `ffmpeg -y -i "${outputPath}" -t 30 -c copy "${output30s}"`;
      await execPromise(short30Cmd);
      console.log('✅ Created 30-second version');
      
      // Create thumbnail
      const thumbnail = path.join(__dirname, '..', 'output', 'mobile-4user-demo-thumbnail.jpg');
      const thumbCmd = `ffmpeg -y -i "${outputPath}" -vframes 1 -f image2 "${thumbnail}"`;
